...
interceptLogCalls(scriptContent, timeout)
...
```js
function decrypt(clearMsgAb, cipheredMsgAb, clearMsgBc, cipheredMsgBc, cipheredMsgCba) {
    ...
    const decryptedMsg = applySubstitution(aliceBobMap, applySubstitution(bobCarolMap, cipheredMsgCba));
...
const decryptedMsg = decrypt(clearMsgAb, cipheredMsgAb, clearMsgBc, cipheredMsgBc, cipheredMsgCba);
...
const cipheredMsgCba = "YCARHXMKZOTJUSTNWBKIWLFTDEKVQEPGSOJNTYNNJNCKVJFQDFWSORSRDTORVJNWXUC";
